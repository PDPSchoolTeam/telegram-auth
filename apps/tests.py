...
def create_user(db):
    ...
    return User.objects.create(
        telegram_id=34,  # Test case bilan mos kelishi kerak
        username="test_user",
        first_name="John",
        last_name="Doe"
    )
...
@pytest.fixture
def api_client():
    """Django test client uchun fixture"""
...
def test_create_user_via_telegram_api(api_client):
    """Telegram API orqali foydalanuvchi ro‘yxatdan o‘tishini test qilish"""
    new_user_data = {
        "first_name": "Alice",
        "last_name": "Smith",
        "username": "new_user",
    ...
    url = reverse("telegram-auth")  # `urls.py` da nomlangan `path` kerak
...
def test_create_user_invalid_data(api_client):
    """Noto‘g‘ri foydalanuvchi ma’lumotlarini tekshirish"""
    invalid_data = {"telegram_id": "incomplete_user"}  # `telegram_id` yetishmaydi
    url = reverse("telegram-auth")
    response = api_client.post(url, invalid_data)

    assert response.status_code == status.HTTP_400_BAD_REQUEST
    assert "telegram_id" in response.data  # Validatsiya xatosi borligini tekshiramiz


@pytest.mark.django_db
def test_get_all_users(api_client, create_user):
    """Barcha foydalanuvchilar ro‘yxatini olishni test qilish"""
    url = reverse("telegram-user-list")
    response = api_client.get(url)

    assert response.status_code == status.HTTP_200_OK
    assert len(response.data) >= 1  # Kamida bitta foydalanuvchi borligini tekshiramiz


@pytest.mark.django_db
def test_get_specific_user(api_client, create_user):
    """Telegram ID orqali ma’lum bir foydalanuvchini olishni test qilish"""
...
def test_get_non_existent_user(api_client):
    """Mavjud bo‘lmagan foydalanuvchini olish testi"""
    url = reverse("telegram-user-detail", kwargs={"pk": 99999999})  # Yaroqsiz `telegram_id`
    response = api_client.get(url)
